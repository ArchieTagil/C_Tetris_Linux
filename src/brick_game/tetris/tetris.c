...
void userInput(UserAction_t action, int hold) {
  ...
  if (action == Left) {
    ...
    if (current_figure->x + i_left > 0 &&
        sideway_collision(*current_figure, action) == 0) {
      current_figure->x--;
    }
  }
  if (action == Right) {
...
void perform_spawn(GameInfo_t *game_info, figure *current_figure,
                   tetris_state *state) {
  ...
  matrix_pointer_copy(&current_figure->tmp_field, game_info->field, ROWS_COUNT,
                      COLS_COUNT);
  current_figure->x = 3;
  current_figure->y = 0;
...
void perform_moving(GameInfo_t *game_info, figure *current_figure,
                    tetris_state *state) {
  ...
  if (collision_code == 0) {
    ...
    *state = SPAWN;
  matrix_pointer_copy(&game_info->field, current_figure->tmp_field, ROWS_COUNT,
                      COLS_COUNT);
  place_figure_on_the_field(game_info, *current_figure);
...
void rotate_figure(figure *current_figure) {
  ...
  figure_copy(&tmp_matrix,
              current_figure->figures[current_figure->current_figure_id]
                                     [new_rotate_index]);
  if (is_rotate_collision(tmp_matrix, *current_figure) == 0) {
    current_figure->rotate_value = new_rotate_index;
    figure_copy(&current_figure->figure_field,
                current_figure->figures[current_figure->current_figure_id]
                                       [current_figure->rotate_value]);
  }
...
int is_rotate_collision(int **matrix, figure current_figure) {
  ...
  for (int i = 0; i < FIGURE_ROWS; i++) {
    for (int j = 0; j < FIGURE_COLS; j++) {
      if (matrix[i][j] == 1) {
        if (i + current_figure.y > 19 || j + current_figure.x < 0 ||
            j + current_figure.x > 9) {
          result = 1;
        } else {
          if (current_figure
                  .tmp_field[i + current_figure.y][j + current_figure.x] == 1) {
            result = 1;
          }
        }
      }
  ...
  for (int i = 0; i < FIGURE_ROWS; i++) {
    for (int j = 0; j < FIGURE_COLS; j++) {
      if (current_figure.figure_field[i][j] == 1) {
        if (current_figure.tmp_field[i + current_figure.y + 1]
                                    [j + current_figure.x] == 1) {
...
void place_figure_on_the_field(GameInfo_t *game_info, figure current_figure) {
  for (size_t i = 0; i < FIGURE_ROWS; i++) {
    for (size_t j = 0; j < FIGURE_COLS; j++) {
      if (current_figure.figure_field[i][j] == 1)
        game_info->field[i + current_figure.y][j + current_figure.x] =
            current_figure.figure_field[i][j];
    }
  }
}

void shift_down(figure *current_figure) { current_figure->y++; }