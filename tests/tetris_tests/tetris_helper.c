...
START_TEST(test_sideway_collision_1) {
  ...
  for (size_t i = 0; i < ROWS_COUNT;
       i++) {  //рисуем стенки по бокам поля для имитации столкновения
...
START_TEST(test_clean_figure) {
  ...
  for (size_t i = 0; i < ROWS_COUNT;
       i++) {  //проверяем, что временное поле всё из нулей
    for (size_t j = 0; j < COLS_COUNT; j++) {
...
START_TEST(test_matrix_pointer_copy) {
  ...
  for (size_t i = 0; i < ROWS_COUNT;
       i++) {  //проверяем, что временное поле всё из единиц
    for (size_t j = 0; j < COLS_COUNT; j++) {
